...
def clock_in_route():
    ...
    role = claims.get("role")
    if role not in ["technician", "contractor", "employee", "admin", "manager"]:
        return jsonify({"error": "Unauthorized"}), 403
    data = request.get_json(silent=True) or {}
    start_time = data.get("start_time")
    try:
        entry = service.clock_in(user_id, tenant_id, start_time=start_time, kind=data.get("kind"), task_id=data.get("task_id"), notes=data.get("notes"))
        return jsonify({
            "id": entry.id,
            "user_id": entry.user_id,
            "start_time": entry.start_time.isoformat(),
...
def get_timesheet_route(user_id):
    ...
    tenant_id = request.args.get("tenant_id") or claims.get("tenant_id")
    role = claims.get("role")
    if requester_id != user_id and role not in ["manager", "admin", "superadmin"]:
        return jsonify({"error":"Unauthorized"}), 403
    start_date = request.args.get("start_date")
    end_date = request.args.get("end_date")
    try:
...
def get_shifts_route():
    ...
    q = service.get_shifts_query(tenant_id)  # service helper returns query
    if start:
        try:
            start_dt = datetime.fromisoformat(start.replace("Z", "+00:00"))
            q = q.filter(service.Shift.start_time >= start_dt)
        except Exception:
            pass
    if end:
        try:
            end_dt = datetime.fromisoformat(end.replace("Z", "+00:00"))
            q = q.filter(service.Shift.end_time <= end_dt)
        except Exception:
            pass
    shifts = q.order_by(service.Shift.start_time.desc()).all()
    return jsonify({"data":[
        {
            "id": s.id,
            "start_time": s.start_time.isoformat(),
...
def create_shift_route():
    ...
    try:
        start_time = datetime.fromisoformat(data["start_time"].replace("Z", "+00:00"))
        end_time = datetime.fromisoformat(data["end_time"].replace("Z", "+00:00"))
        shift = service.create_shift(tenant_id, start_time, end_time,
                                    role=data.get("role"), team=data.get("team"),
...
def assign_shift_users_route(shift_id):
    ...
    role = claims.get("role")
    if role not in ["manager", "admin", "superadmin"]:
        return jsonify({"error":"Unauthorized: only managers or admins can assign users"}), 403
    data = request.get_json() or {}
    user_ids = data.get("user_ids", [])
    try:
        result = service.assign_users(shift_id, user_ids)
