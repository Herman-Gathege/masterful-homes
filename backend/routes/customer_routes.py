...
from legacy_models import db, LegacyCustomer, LegacyInstallation, LegacyInvoice
...
def list_customers(current_user):
    ...
    query = LegacyCustomer.query
    if status:
        query = query.filter_by(status=status)

    pagination = query.order_by(LegacyCustomer.id.desc()).paginate(
...
def customer_360(current_user, customer_id):
    customer = LegacyCustomer.query.get(customer_id)
    if not customer:
